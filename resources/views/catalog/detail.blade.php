...
<div class="max-w-6xl mx-auto px-4 py-10">
    ...
    <nav class="text-sm text-red-500 mb-6">
        ...
        <a href="/books" class="hover:text-white font-bold">Buku</a> /
    ...
    <div class="bg-zinc-950 border border-red-900/50 rounded-2xl shadow p-6 md:p-10 flex flex-col md:flex-row gap-10 mb-10">
        ...
        {{-- Info --}}
        <div class="flex-1">
            <span class="text-xs text-white font-bold font-medium bg-indigo-50 px-3 py-1 rounded-full">
                {{ $book->category->name }}
            </span>
            ...
            @if($book->stock > 0)
                @if(session('user') && session('user')['role'] === 'user')
                    <form action="/cart" method="POST" class="flex items-center gap-3">
                        ...
                        <button type="submit"
                                class="bg-red-600 font-bold text-white text-white px-8 py-2 rounded-lg font-semibold
                ...
                @elseif(!session('user'))
                    <a href="/login"
                       class="inline-block bg-red-600 font-bold text-white text-white px-8 py-2 rounded-lg
            ...
            @else
                <button disabled
                        class="bg-gray-300 text-red-500 px-8 py-2 rounded-lg font-semibold cursor-not-allowed">
                    Stok Habis
                </button>
            @endif
        </div>
